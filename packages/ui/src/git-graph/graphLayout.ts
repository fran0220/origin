...
function laneToCommitLine(
	...
	return {
		childColumn: lane.startingCol,
		startRow: lane.startingRow,
		endRow: endingRow,
		colorIdx: finalColor,
		segments,
	};
...
export function computeGraphLayout(nodes: readonly GraphCommitNode[], colorsCount: number): GraphLayout {
	...
	for (let commitRow = 0; commitRow < nodes.length; commitRow++) {
		...
		commit.parents.forEach((parent, parentIdx) => {
			if (parentIdx === 0) {
				...
				let firstParentLanes = parentToLanes.get(parent);
				if (!firstParentLanes) {
					firstParentLanes = [];
					parentToLanes.set(parent, firstParentLanes);
				}
				firstParentLanes.push(commitLane);
			} else {
				...
				let extraParentLanes = parentToLanes.get(parent);
				if (!extraParentLanes) {
					extraParentLanes = [];
					parentToLanes.set(parent, extraParentLanes);
				}
