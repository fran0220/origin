...
All notable changes to `@origin/runtime-node` are documented in this file.

## [Unreleased]

### Added

- 新增本地模型网关会话亲和性 Stream 适配器，由 Node 宿主显式把 Runtime Session 映射为
  `X-Session-ID`；远程 Provider 请求与调用方已有 Header 保持不变。
...
- 文档转换与 OCR 的 Node 组合收敛为 `createNodeSpecializedToolRegistrations()`，并由 Node Tool Environment
  ...
  `createAsyncExecutionGate()` 迁至 `@origin/runtime-tools`，不再从 `@origin/runtime-node/coding` 导出。

- 移除 `@origin/runtime-node/coding` 的 `current_time`、`progress`、`task_output`、`task_stop` Tool 定义、Schema、
...
- 移除 `@origin/runtime-node/coding` 的 `im_send_attachment` 产品 Tool 与注册导出；新增通用
  `createNodeFileInspectionOperations()` Host 实现，最终 CLI Host 将它注入 Coding Agent IM Feature。

- 移除 `@origin/runtime-node/coding` 的 7 个子代理控制 Tool（`spawn_agent`、`dispatch_workflows`、`wait_agent`、
  `list_agents`、`interrupt_agent`、`send_message`、`followup_task`）及其 Schema、描述和注册导出。它们不依赖
  Node 环境，现由 `@origin/coding-agent` 的 Subagent Feature 持有；同属产品协议的子代理完成通知投影也一并迁移，
  Node Runtime 继续只提供平台实现。

- 移除 `@origin/runtime-node/coding` 中无人使用的重复 `tool_search` Tool、评分器、Schema 与注册导出。
  会话级渐进披露的唯一生产实现和公共合同位于平台中立的 `@origin/runtime-mcp`。

- 移除 `@origin/runtime-node/coding` 的 `invoke_skill` Tool、Schema、描述和注册导出。Skill 已在进入
  Coding Agent 领域时完成物化，该模型可见语义现由 `@origin/coding-agent` 的 Skill 领域持有；Node Runtime
  不再承载无平台依赖的产品 Tool 定义。

- 移除 `@origin/runtime-node/coding` 的 `ask_user_question` Tool、Schema、描述、Capability 类型和注册导出。
  该模型可见交互语义现由 `@origin/coding-agent` 的 Ask User Question Feature 持有，并直接消费
  `runtime-core` 的通用宿主提问 Capability；用户行为保持不变。

- 移除 `@origin/runtime-node/coding` 的 `memory` Tool、Schema、描述和注册导出。模型可见 Memory 语义现由
  `@origin/coding-agent` 拥有；Node Runtime 只提供通用文本文件存储适配器。

- 移除 `@origin/runtime-node/coding` 的 `kb_list_available_tags`、`kb_filter_by_tags` 与 `kb_write_page`
  Tool 定义及其 Schema/注册导出。这些模型可见语义现由 `@origin/coding-agent` 的 Knowledge Feature 拥有；Node Runtime
  只提供显式根目录的查询与写入适配器。

- `@origin/runtime-node/sandbox` 不再导出带 Coding Agent UI 文案和授权决策的 `confirmSandboxPermission()` 与
  `isSensitiveSandboxRequest()`；确认策略现由调用方拥有，Node Runtime 只提供路径、Grant 和 OS 执行能力。

- 移除 `@origin/runtime-node/coding` 的 Todo Tool、Schema 和注册导出。Todo 不访问 Node 环境且包含明确
  产品规则，现由 `@origin/coding-agent` 的 Todo Feature 直接拥有；Coding Agent 用户行为保持不变。
...
- `@origin/runtime-node/coding` 新增 `createNodeSandboxCodingToolEnvironment()`，从显式路径策略与 Host
  选项统一创建 `read`、`write`、`edit`、平台命令 Tool 和 Node sandbox services；上层产品只需装饰权限语义，
  不再重复选择具体 Tool 构造器。

- `@origin/runtime-node/coding` 新增 `createNodeCommandToolEnvironment()` 与
...
- `@origin/runtime-node/coding` 新增 `createNodePathBoundaryClassifier()` 与
...
- `@origin/runtime-node/host` 新增 `createNodeKnowledgeRuntime(root)`，把 `runtime-knowledge` 的 Tag 查询、页面写入和
...
- **Resource Package SHA-256 适配器**：`@origin/runtime-node/host` 新增无状态 `nodeResourcePackageDigest`，供上层位置策略
...
- **Node MCP 适配器**：从 `@origin/runtime-mcp` 接管文件配置、OAuth 状态文件、Vetta credentials、具体 Client Factory、stdio/HTTP transport、SDK OAuth Provider、Device Flow 与内置 Vetta MCP；协议包继续拥有 Port、Schema、Supervisor 状态机、Tool 投影和渐进披露。
- **Node Coding Tool 适配器**：从 `@origin/runtime-tools` 接管具体 Tool 工厂、TypeBox Schema、模型描述、文件/命令/PDF/OCR 行为及 Node Host 原语；协议包继续拥有 Catalog、注册、激活、绑定和结果策略合同。
- **Node Conversation 适配器**：从 `@origin/runtime-storage` 接管文件与内存 Repository、所有权租约、原子文件发布、会话目录、持久化编解码及 Legacy 会话文件读取和迁移；保持原有并发、恢复、续接与兼容行为。
