...
const CODING_TOOL_RELEASES: Record<CodingToolExecutable, CodingToolReleaseConfig> = {
	fd: {
		...
		resolveAssetName: (version, platform, architecture) => {
			...
			if (platform === "darwin") return `fd-v${version}-${arch}-apple-darwin.tar.gz`;
			if (platform === "linux") return `fd-v${version}-${arch}-unknown-linux-gnu.tar.gz`;
			if (platform === "win32") return `fd-v${version}-${arch}-pc-windows-msvc.zip`;
			return undefined;
		},
	},
	rg: {
		...
		resolveAssetName: (version, platform, architecture) => {
			const arch = architecture === "arm64" ? "aarch64" : "x86_64";
			if (platform === "darwin") return `ripgrep-${version}-${arch}-apple-darwin.tar.gz`;
			if (platform === "linux") {
				return architecture === "arm64"
					? `ripgrep-${version}-aarch64-unknown-linux-gnu.tar.gz`
					: `ripgrep-${version}-x86_64-unknown-linux-musl.tar.gz`;
			}
			if (platform === "win32") return `ripgrep-${version}-${arch}-pc-windows-msvc.zip`;
			return undefined;
		},
	},
};
