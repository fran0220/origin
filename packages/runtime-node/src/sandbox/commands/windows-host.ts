...
import { buildWindowsSandboxPolicy } from "./windows-policy.js";
...
export function resolveWindowsSandboxHostPath(explicitPath?: string): string {
	const moduleDir = dirname(fileURLToPath(import.meta.url));
	const runtimeCoreRootCandidates = [
		resolvePath(moduleDir, "../.."),
		resolvePath(moduleDir, "../../../packages/runtime-core"),
		resolvePath(moduleDir, "../../../runtime-core"),
		resolvePath(moduleDir, "../../../../runtime-core"),
...
function resolveWindowsShellCommand(): { command: string; args: string[] } {
	...
	if (pwshPath) return { command: pwshPath, args: ["-NoLogo", "-NoProfile", "-NonInteractive", "-Command"] };
	const powershellPath = findOnPath("powershell.exe");
	if (powershellPath) {
		return { command: powershellPath, args: ["-NoLogo", "-NoProfile", "-NonInteractive", "-Command"] };
	}
	return { command: findOnPath("cmd.exe") ?? "cmd.exe", args: ["/d", "/s", "/c"] };
}

export function createWindowsSandboxCommandOperations(sandboxHostPath?: string): ForegroundCommandOperations {
	const resolvedSandboxHostPath = resolveWindowsSandboxHostPath(sandboxHostPath);
	return {
		exec: (command, cwd, { onData, signal, timeout, env }) =>
			new Promise<{ exitCode: number | null }>((resolve, reject) => {
				void (async () => {
					...
					for (const root of policy.denyReadRoots) args.push("--deny-read-path", root);
					for (const root of policy.denyWriteRoots) args.push("--deny-write-path", root);
					if (typeof timeout === "number" && timeout > 0) {
						args.push("--timeout-ms", String(Math.max(1, Math.floor(timeout * 1000))));
					}
					const resolvedCommand = prependCommandPrefixes(command, [
						getNodeShellCommandPrefix(shellCommand.command),
					]);
					args.push("--", shellCommand.command, ...shellCommand.args, resolvedCommand);
					const child = spawn(resolvedSandboxHostPath, args, { cwd, stdio: ["ignore", "pipe", "pipe"] });
					child.stdout?.on("data", onData);
					child.stderr?.on("data", onData);

					const cleanup = (): void => void rm(tempRoot, { recursive: true, force: true });
