import type { CommandProcessPort, DesktopCommandResult } from "@origin/runtime-node/coding";
import { describe, expect, it } from "vitest";
import { createNodeDocToPdfOperations } from "../../src/coding/host/doc-to-pdf-operations.js";

describe("Node doc-to-pdf host operations", () => {
	it("detects macOS Office installations through the file-system port", async () => {
		const operations = createNodeDocToPdfOperations({
			platform: "darwin",
	...
	it("builds platform conversion commands and reports unsupported combinations", async () => {
		...
		const macOperations = createNodeDocToPdfOperations({
			platform: "darwin",
		...
		const unsupported = createNodeDocToPdfOperations({
			platform: "freebsd",
			commandProcess: processFixture([]).port,
		});
