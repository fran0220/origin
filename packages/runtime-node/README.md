...
- `@origin/runtime-node/conversation`：文件/内存 Repository、租约、原子发布、会话服务与 Legacy 迁移
- `@origin/runtime-node/coding`：具体 Coding Tool、Schema、模型描述、文件/命令/PDF/OCR 实现与 Node Host 原语
- `@origin/runtime-node/mcp`：文件配置与凭证、stdio/HTTP Client、MCP SDK/OAuth 和 Device Flow 实现
- `@origin/runtime-node/host`：资源、Knowledge、结果制品与通用文本文件等 Node Host 适配器
