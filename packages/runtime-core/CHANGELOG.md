...
- `ModelCallFrame` 新增可选字段 `systemPromptStableLength`：由产品级 Frame Composer 声明系统提示词稳定前缀长度，`resolveModelCallFrame` 原样透传，Turn Engine 写入 `Context.systemPromptStableLength` 供 Provider 切缓存断点。`instructionOverride` 生效时该值被显式丢弃（override 替换了整段 Prompt，偏移不再成立）。
