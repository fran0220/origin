...
export const ACTION_RPC_ENDPOINT_FILE_ENV = "ORIGIN_ACTION_RPC_ENDPOINT_FILE";
export const ORIGIN_HOME_ENV = "ORIGIN_HOME";
export const ORIGIN_CONFIG_DIR_ENV = "ORIGIN_CONFIG_DIR";

/**
 * 默认配置目录名（即品牌名）。这是整个仓库配置目录的唯一来源——
 * 既用于用户主目录根（~/<name>），也用于项目内目录（projectDir/<name>）。
 * 改品牌只改这一处；按环境隔离（如 dev）则用 ORIGIN_CONFIG_DIR 覆盖。
 */
export const DEFAULT_CONFIG_DIR_NAME = ".origin";
...
/**
 * 配置目录名。ORIGIN_CONFIG_DIR 覆盖，否则用默认品牌名。
 * 用户主目录根与项目内目录都从它派生，保证两者始终一致。
 */
export function getOriginConfigDirName(): string {
	return process.env[ORIGIN_CONFIG_DIR_ENV] || DEFAULT_CONFIG_DIR_NAME;
}

/**
 * 用户主目录下的数据根。ORIGIN_HOME（绝对路径，支持 ~ 展开）作为逃生口优先，
 * 否则为 ~/<configDirName>。
 */
export function getOriginHomePath(): string {
	const explicit = process.env[ORIGIN_HOME_ENV];
	if (explicit) return expandTilde(explicit);
	return join(homedir(), getOriginConfigDirName());
}

export function getActionRpcEndpointFilePath(): string {
	const envPath = process.env[ACTION_RPC_ENDPOINT_FILE_ENV];
	if (envPath) return envPath;
	return join(getOriginHomePath(), "action-server.json");
}
