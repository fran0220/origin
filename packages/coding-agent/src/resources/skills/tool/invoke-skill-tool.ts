...
export interface InvokeSkillToolOptions {
	...
	readonly readBody: (skill: Skill, request: RuntimeToolExecutionRequest<InvokeSkillToolInput>) => string;
}

export function createInvokeSkillTool(options: InvokeSkillToolOptions): RuntimeToolDefinition<InvokeSkillToolInput> {
	return {
		name: "invoke_skill",
		label: "invoke_skill",
