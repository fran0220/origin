...
import type { ExtensionRunnerGenerationOwner, ExtensionRunnerLease } from "./extension-runner-generations.js";
import { createExtensionToolInterceptor, wrapRuntimeToolsWithExtensions } from "./extension-tool-interceptor.js";
 ...
 */
export class CodingAgentExtensionRunBridge implements AgentRunPreparer {
	...
	bindForTurn(context: RuntimeSnapshotAcquireContext): CodingAgentExtensionRunBridge {
		return this.bindAdapterForTurn(context);
	}

	bindAdapterForTurn(context: RuntimeSnapshotAcquireContext): CodingAgentExtensionRunBridge {
		this.boundSessionId ??= context.sessionId;
		const lease = this.runnerGenerations?.acquire(context.sessionId, context.operationId);
		const bound = new CodingAgentExtensionRunBridge();
