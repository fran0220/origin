import {
	...
	ORIGIN_CLIENT_VERSION_HEADER,
	type OriginCredentials,
} from "@origin/runtime-node/mcp";
import { describe, expect, it } from "vitest";

const CREDENTIALS: OriginCredentials = { baseUrl: "https://api.example.com", token: "tok-1" };

function build(credentials: OriginCredentials | null, clientVersion = "1.2.3") {
...
describe("buildBuiltinMcpServers", () => {
	it("组装指向服务端 MCP endpoint 的 http 配置", () => {
		const config = build(CREDENTIALS)[ORIGIN_BUILTIN_MCP_NAME];
	...
	it("baseUrl 自带 API 前缀时不重复拼接", () => {
		// 桌面端注入的 ORIGIN_SERVER_URL 本身就带 /api/v1，手工配的通常只到域名，
		// 不归一就会拼出 /api/v1/api/v1/mcp 而 404
		const config = build({ baseUrl: "https://api.example.com/api/v1", token: "t" })[ORIGIN_BUILTIN_MCP_NAME];
	...
	it("带客户端版本头", () => {
		...
		const config = build(CREDENTIALS, "0.6.0")[ORIGIN_BUILTIN_MCP_NAME];

		expect(config.headers?.[ORIGIN_CLIENT_VERSION_HEADER]).toBe("0.6.0");
	});
	...
	it("运行时名不含下划线", () => {
		...
		expect(ORIGIN_BUILTIN_MCP_NAME).not.toContain("_");
	});
	...
	it("每次解析 header 都重读凭据，拿到轮换后的 token", () => {
		let current: OriginCredentials | null = { baseUrl: "https://api.example.com", token: "old" };
		const servers = buildBuiltinMcpServers({ loadCredentials: () => current });
		const resolve = servers[ORIGIN_BUILTIN_MCP_NAME].resolveHeaders;
	...
	it("登出后不再带 Authorization", () => {
		...
		let current: OriginCredentials | null = CREDENTIALS;
		const servers = buildBuiltinMcpServers({ loadCredentials: () => current });
		const resolve = servers[ORIGIN_BUILTIN_MCP_NAME].resolveHeaders;
