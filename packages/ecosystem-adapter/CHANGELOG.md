...
All notable changes to `@origin/ecosystem-adapter` are documented in this file.
...
- **Hook 配置路径发现 `buildDefaultHookConfigLayers()`**：仅在 Vetta 根下镜像官方 `.codex` / `.claude` 布局——用户：`~/.origin/.codex/hooks.json`、`~/.origin/.claude/settings.json`；项目：`<cwd>/.origin/.codex/hooks.json`、`<cwd>/.origin/.claude/settings.json`、`settings.local.json`。source 带 `profileId` 隔离；**不读**顶层 `~/.codex` / `~/.claude` 或项目根 `.codex` / `.claude`，避免加载无关官方 hook。
