...
export function validateArticle(articlePath, options = {}) {
	...
	const marked = /<!--\s*origin-blog:\s*article\s*-->/i.test(article) || /^\s*origin-blog:\s*(?:true|article)\s*$/im.test(article);
	checks.marker = marked;
	if (!marked) errors.push("缺少 `<!-- origin-blog: article -->` 标记，hook 不会把它识别为博客正文。");
	...
	const distinctTerms = ORIGIN_TERMS.filter((term) => article.toLocaleLowerCase().includes(term.toLocaleLowerCase()));
	checks.originRelevance = distinctTerms.length >= 2;
...
function renderFeedback(result) {
	const lines = [`origin-blog ${result.verdict}（${result.score}/16）`];
...
async function runHook(input) {
	...
	for (const candidate of candidates) {
		...
		if (!/<!--\s*origin-blog:\s*article\s*-->|^\s*origin-blog:\s*(?:true|article)\s*$/im.test(text)) continue;
