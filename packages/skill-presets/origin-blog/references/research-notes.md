...
需要主动避免的 OpenDesign 风险：内部产品宣传占比过高；用“截至撰写本文时”却没有检索日期；竞品数据（星数、价格、兼容性）容易过期；同一判断在开头、正文、结尾重复；版本文章可能把未验证的实现细节包装成稳定合同。`origin-blog` 将这些变成证据日期、claim 状态和限制段落，而不是禁止所有产品观点。
