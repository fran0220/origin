...
describe("batch task domain capabilities", () => {
	it("uses one stable id per batch task operation", () => {
		expect(Object.values(DOMAIN_BATCH_TASK_CAPABILITIES).map((capability) => capability.id)).toEqual([
			`${CAPABILITY_PREFIXES.ORIGIN_DOMAIN}batch-task.project.list`,
			`${CAPABILITY_PREFIXES.ORIGIN_DOMAIN}batch-task.project.get`,
			`${CAPABILITY_PREFIXES.ORIGIN_DOMAIN}batch-task.project.create`,
			`${CAPABILITY_PREFIXES.ORIGIN_DOMAIN}batch-task.project.update`,
			`${CAPABILITY_PREFIXES.ORIGIN_DOMAIN}batch-task.project.delete`,
			`${CAPABILITY_PREFIXES.ORIGIN_DOMAIN}batch-task.task.run`,
			`${CAPABILITY_PREFIXES.ORIGIN_DOMAIN}batch-task.task.retry`,
			`${CAPABILITY_PREFIXES.ORIGIN_DOMAIN}batch-task.task.stop`,
			`${CAPABILITY_PREFIXES.ORIGIN_DOMAIN}batch-task.task.delete`,
			`${CAPABILITY_PREFIXES.ORIGIN_DOMAIN}batch-task.task.resume`,
			`${CAPABILITY_PREFIXES.ORIGIN_DOMAIN}batch-task.task.resume-with-text`,
			`${CAPABILITY_PREFIXES.ORIGIN_DOMAIN}batch-task.task.session.delete`,
			`${CAPABILITY_PREFIXES.ORIGIN_DOMAIN}batch-task.project.task.delete-all`,
			`${CAPABILITY_PREFIXES.ORIGIN_DOMAIN}batch-task.project.start`,
			`${CAPABILITY_PREFIXES.ORIGIN_DOMAIN}batch-task.project.stop`,
			`${CAPABILITY_PREFIXES.ORIGIN_DOMAIN}batch-task.project.reset`,
			`${CAPABILITY_PREFIXES.ORIGIN_DOMAIN}batch-task.project.failed-task.reset`,
		]);
	});
