...
`@origin-org/plugin-vite` creates the archive automatically after `vite build`; no separate packaging script is required.
...
- `@origin-org/plugin-vite` supplies the default Vite Module Federation and Rollup configuration for Vetta plugins.
...
- `@origin-org/plugin-sdk` is provided by the host and remains external.
