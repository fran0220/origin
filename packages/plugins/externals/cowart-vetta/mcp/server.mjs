...
function registerCowartVettaOpenTool(mcpServer) {
  mcpServer.registerTool(
    ...
    async ({ projectDir, canvasDir }) => {
      const paths = resolveCowartPaths({ projectDir, canvasDir });
      return {
        content: [
          {
            type: "text",
            text: [
              ...
              "Use get_cowart_canvas_state, get_cowart_selection, insert_cowart_image, and related MCP tools for agent-side canvas IO.",
            ].join("\n"),
          },
        ],
        structuredContent: {
          mode: "vetta",
          widget: "cowart-activity-tab",
          projectDir: paths.projectDir,
          canvasDir: paths.canvasDir,
        },
      };
