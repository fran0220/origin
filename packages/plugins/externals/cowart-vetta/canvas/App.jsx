...
const AI_IMAGE_GENERATION_PROMPT_PREFIX = [
  ...
  '插入多张图片时，第一张 insert_cowart_image 替换 AI 图片框；之后每次使用上一张插入结果返回的 shapeId 作为 anchorShapeId，并设置 replaceAiImageHolder: false、matchAnchor: false、placement: "right"。',
  '不要把多张图片合成一张拼图、画册或带分页的单一产物。',
  '如果附带一张或多张参考图，请把参考图作为视觉参考传给 generate_image；不要把参考图文件名或任何界面元素画进最终图片。',
...
function resolveCowartBundledAssetUrl(url) {
  ...
  try {
    if (url.startsWith('/assets/')) {
      return new URL(url.slice('/assets/'.length), import.meta.url).href
    }
    if (url.startsWith('/')) {
      return new URL(url.slice(1), new URL('../', import.meta.url)).href
    }
...
function rewriteCowartAssetUrlTree(value) {
  if (typeof value === 'string') return resolveCowartBundledAssetUrl(value)
  if (Array.isArray(value)) return value.map(rewriteCowartAssetUrlTree)
  if (value && typeof value === 'object') {
    const out = {}
    for (const [key, child] of Object.entries(value)) {
      out[key] = rewriteCowartAssetUrlTree(child)
    }
    return out
  }
  return value
...
function buildCowartAssetUrls() {
  ...
  let base = {}
  try {
    base = rewriteCowartAssetUrlTree(getAssetUrlsByImport())
  } catch (error) {
...
function buildAnnotationEditPrompt({ imageShapeId, shapeIds, exportWidth, exportHeight, screenshotAsset }) {
  ...
  if (screenshotAsset?.assetPath) {
    lines.push(
      ...
      'Use this local screenshot as the source input for edit_image (authoritative visual + annotation brief).'
    )
  }
  ...
  lines.push(
    '',
    'Required tool flow:',
    '- Call edit_image with the annotation screenshot (and original image if available) to produce a clean revised bitmap.',
    `- Call insert_cowart_image with anchorShapeId: "${imageShapeId}", placement: "right", margin: 40, matchAnchor: true, replaceAiImageHolder: false.`,
    '- Leave the original image shape and annotations unchanged.'
  )
...
async function sendAnnotationEditRequest(editor, imageShapeId) {
  ...
  if (!sender) {
    throw new Error('当前 Cowart 画布没有可用的 Cowart MCP 消息桥。')
  }
...
async function sendAnnotationHtmlRequest(editor, imageShapeId) {
  ...
  const sender = followUpSender()
  if (!sender) throw new Error('当前 Cowart 画布没有可用的 Cowart MCP 消息桥。')
...
function buildHtmlDraftAnnotationImagePrompt({ draftShape, exportResult, screenshotAsset }) {
  ...
  return [
    ...
    '',
    'Required tool flow:',
    '- Call generate_image (not edit_image) to create a new clean bitmap from the HTML draft + annotations.',
...
function buildAiImageGenerationPrompt({ holderShape, userPrompt, references, referenceAttached }) {
  ...
  return [
    ...
    '',
    'Required tool flow:',
    '- Call generate_image for each new bitmap (size/aspect should match the target slot when the tool allows).',
    `- Call insert_cowart_image with anchorShapeId: "${holderShape.id}" and replaceAiImageHolder: true for the first image so the AI holder becomes a normal image shape.`,
    '- For additional images: replaceAiImageHolder: false, matchAnchor: false, placement: "right", using the previous inserted shapeId as anchorShapeId.',
    '',
    'Prompt:',
    userPrompt.trim()
  ].join('\n')
  ...
  if (loadError) {
    ...
    return (
      <main className="cowart-status" aria-live="polite">
        ...
        {message ? <div style={{ marginTop: 8, maxWidth: 480, fontSize: 12, opacity: 0.8 }}>{message}</div> : null}
      </main>
    )
  }
