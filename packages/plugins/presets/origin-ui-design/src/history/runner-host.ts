 ...
 * 路径；插件的 `fs.write` 又是项目作用域的，写不到 `~/.origin`。引擎模板是同一个问题
 ...
 * 异步 chunk 经 origin-plugin:// 加载的机制是可用的：宿主协议按 standard scheme
...
async function materialize(ctx: PluginContext): Promise<string> {
	...
	const dir = `${home}/.origin/plugin-data/origin-ui-design/history-runner/${hash}`;
