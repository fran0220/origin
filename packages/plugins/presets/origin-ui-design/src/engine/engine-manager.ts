 ...
 *
 * 1. Migrate the legacy ~/.origin/design-engine directory into this plugin's
...
function engineBaseDir(home: string): string {
	return `${home}/.origin/plugin-data/origin-ui-design/design-engine`;
}

function legacyEngineBaseDir(home: string): string {
	return `${home}/.origin/design-engine`;
}
