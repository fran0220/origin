...
function statusNote(
	...
	if (frameCount > 1 && shell && !shell.layout && shell.components.length === 0) {
		return "Multiple screens but NO shared UI yet. If they share a nav bar / sidebar / tab bar, extract it into components/ (or frames/_layout.tsx when it must survive navigation) rather than repeating it per frame. The structure checklist is in the origin-ui-design skill, under Workflow.";
	}
...
export function registerDesignTools(ctx: PluginContext): void {
	ctx.agent.registerTool<CreateInput>({
		...
		handler: async ({ host, session, trigger, actions }) => {
			...
			console.debug(
				`[origin-ui-design:activity-tab-debug] vetd_create requesting canvas ${JSON.stringify({
			...
			return {
				...
				defaultFrameSize,
				note: `Design created and opened on the canvas, with NO frames yet. Its default size is ${defaultFrameSize.width}x${defaultFrameSize.height} — still declare it per frame: every frames/<id>.tsx starts with \`export const frame = { width: ${defaultFrameSize.width}, height: ${defaultFrameSize.height}, title }\`, and a screen of a different product type declares its own. Never edit design.json. More than one screen? Write the shared chrome FIRST (components/, or frames/_layout.tsx), then every frame as a short skeleton so the whole set reaches the canvas immediately, and only then fill them in one at a time — see the origin-ui-design skill.`,
			};
		},
	});
