...
describe("design engine data migration", () => {
	it("moves the legacy engine tree into the plugin data directory", async () => {
		...
		const legacyRoot = join(home, ".origin", "design-engine");
		const targetRoot = join(home, ".origin", "plugin-data", "origin-ui-design", "design-engine");
	...
	it("checks readiness without the project filesystem capability", async () => {
		const home = await temporaryHome();
		const engineRoot = join(home, ".origin", "plugin-data", "origin-ui-design", "design-engine", "0.3.0");
