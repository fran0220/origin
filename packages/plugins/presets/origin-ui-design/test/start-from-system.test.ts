...
describe("buildStyleStartDraft", () => {
	it("只剩意图一句：skill badge + 风格名 + 光标位，跟宿主语言走", () => {
		...
		expect(zh).toBe("@skill:origin-ui-design 请按「Linear」风格设计。我想做：");
		const en = buildStyleStartDraft(system(), "en");
		expect(en).toBe('@skill:origin-ui-design Design this in the "Linear" style. I want to build: ');
	});
...
describe("协议真的住在 skill 里", () => {
	...
	const skillMd = readFileSync(resolve(process.cwd(), "agent/skills/origin-ui-design/SKILL.md"), "utf8");
...
describe("startDesignFromSystem", () => {
	...
	it("资料落盘后写 INDEX.md 清单，草稿只带风格名", async () => {
		...
		expect(navigationOpen).toHaveBeenCalledWith({
			target: "new-session",
			cwd: "/w/my-app",
			draft: "@skill:origin-ui-design 请按「Linear」风格设计。我想做：",
		});
	});
