/**
 * 插件 CSS 编译成 `@scope ([data-origin-plugin-root=origin-ui-design])`，@scope 里的选择器
...
it("作用域根只由 PluginPortal 产出，别处不许自己写", () => {
	const offenders = sourceFiles(join(import.meta.dirname, "../src"))
		.filter((path) => readFileSync(path, "utf8").includes("data-origin-plugin-root"))
...
it("作用域根不带任何 class，样式全部落在子元素上", () => {
	...
	const scopeRoot = document.body.querySelector("[data-origin-plugin-root]");
