...
describe("repoRootUrl", () => {
	it("剥掉清单自身的路径，得到仓库根", () => {
		// 资源地址是相对仓库根的；直接拿清单地址当 base 会多出一段 .origin/。
		expect(repoRootUrl(DESIGN_CATALOG_SOURCES[0])).toBe(
			"https://raw.githubusercontent.com/openvetta/origin-design-templates/main/",
		);
		expect(repoRootUrl(DESIGN_CATALOG_SOURCES[1])).toBe(
			"https://cdn.jsdelivr.net/gh/openvetta/origin-design-templates@main/",
		);
	});
