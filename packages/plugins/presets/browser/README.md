...
Plugin -> ctx.browser -> Plugin Capability Adapter
       -> cap.foundation.origin.browser.* -> Desktop BrowserAutomationService
...
- 每个 Coding Agent Session 获得唯一的 `ORIGIN_AGENT_SESSION_ID`；Skill 要求所有浏览器与页面状态操作用它作为 upstream `--session`，安装、诊断和内置手册读取不绑定 session。
...
同一 Agent 任务内的连续 CLI 命令复用它自己的 session。需要同时操作多个媒体账号时，在 `ORIGIN_AGENT_SESSION_ID` 后追加稳定账号键，并为每个账号使用不同的 `--restore` 键，例如 `youtube-brand-a`、`youtube-brand-b`。关闭活跃 session 不会删除 `--restore` 保存的状态。
