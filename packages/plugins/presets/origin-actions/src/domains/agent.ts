...
import { createOriginActionRegistrar } from "../action-usage";
...
export function registerAgentActions(ctx: PluginContext): void {
	const register = createOriginActionRegistrar(ctx, "agent");
	register<AgentQueryInput>({
		...
		keywords: ["agent", "Agent", "实验", "experimental", "Origin CLI", "输入预测", "技能扩展", "agentSkills"],
		effect: "read",
	...
	register<AgentManageInput>({
		...
		keywords: ["agent", "实验", "experimental", "originCli", "promptPrediction", "agentSkills"],
		effect: "write",
		approval: {
