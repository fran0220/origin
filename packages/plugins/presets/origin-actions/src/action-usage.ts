...
const DOMAIN_USAGE = {
	...
	shortcuts: {
		target: "Origin Desktop 的全局快捷键与快捷面板设置",
		useWhen: "用户要查询或改变 Origin 的快捷键绑定及快捷面板行为。",
		avoidWhen: "实现网页键盘交互、修改 IDE 快捷键或讨论键盘操作。",
		alternatives: "代码交互修改项目；其他应用使用其设置入口；操作咨询直接解释。",
	},
	im: {
		target: "Origin Desktop 的 IM 连接与渠道配置",
		useWhen: "查询或配置 Origin 的飞书、微信等 IM 接入和运行状态。",
		avoidWhen: "发送普通消息或附件、开发聊天软件，或把用户提到的聊天平台自动接入 Origin。",
		alternatives: "当前 IM 附件交付使用 im_send_attachment；消息操作使用对应渠道能力；软件开发修改项目。",
	},
	webhook: {
		target: "Origin Desktop 的 Webhook 推送端点配置",
		useWhen: "用户要查询或配置 Origin 发送通知使用的 Webhook 端点。",
		avoidWhen: "开发业务 Webhook、调用外部 API 或仅要求发送一次消息。",
		alternatives: "业务 Webhook 修改项目；一次性外部操作使用目标服务的能力。",
	},
	downloads: {
		target: "Origin Desktop 下载中心中的任务记录",
		useWhen: "用户要查询或管理 Origin 下载中心已有任务。",
		avoidWhen: "下载任意网页文件、安装项目依赖、查找下载目录文件。",
		alternatives: "文件下载使用对应下载能力；依赖使用包管理器；本地文件使用文件工具。",
	},
	updater: {
		target: "Origin Desktop 应用本身的版本更新",
		useWhen: "用户要检查、下载或安装 Origin 自身的新版，或控制已有更新任务。",
...
export function createOriginActionRegistrar(ctx: PluginContext, domain: keyof typeof DOMAIN_USAGE) {
