...
17 个域、37 个 Action 通过 `src/action-usage.ts` 统一提供 `usage`：作用对象、何时使用、何时不用、替代路径。该说明随 search / describe 返回，帮助区分操作 Vetta 本身与开发用户项目；不参与审批或运行控制。新增域需补充使用说明，已有域使用 `createOriginActionRegistrar` 注册，避免漏掉发现阶段的边界。
