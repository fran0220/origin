---
title: 理解 Origin 的工作方式
description: 用工作区、任务、执行、结果和复用五个概念建立完整的产品心智。
---

<Takeaways>
  <li>Origin 是带边界的本地工作台，不是聊天框</li>
  <li>五项对象构成一次完整工作</li>
  <li>验收看文件和执行记录，不看自信的总结</li>
</Takeaways>

Origin 不是只返回一段答案的聊天框。它把 Agent 放进一个明确的本地工作区，让模型在可检查的权限和工具边界内读取资料、修改文件、执行命令，并把过程和结果保留在会话中。

<MediaFrame>
  <img src="/images/product/workspace.webp" alt="Origin 主工作区，左侧管理项目和会话，中间承载任务输入与结果" width="2354" height="1613" />
  <figcaption>一个工作区同时容纳项目上下文、会话历史、Agent 执行和输出文件。</figcaption>
</MediaFrame>

## 五个核心对象

<ConceptMap aria-label="Origin 核心对象关系">
...
<Compare leftTitle="普通模型对话" rightTitle="Origin 工作流">
