...
`@origin/coding-agent/sdk` 是进程内集成入口。它提供类型化 Session、Host 和离线会话目录，不要求通过 CLI 或解析 stdout。
...
`@origin/runtime-core` 的 `RuntimeHost.installAgent()`。
...
完整类型以 `@origin/coding-agent/sdk` 的声明为准。
