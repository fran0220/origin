...
主题可以从 `@origin-org/theme-sdk/app-shell`、`sidebar` 等公开子路径读取 model hook，再把 model 传给官方 props-driven view。

## regions

Region override 接管一个完整区域，可以重排默认组件、插入主题 UI 并复用 `@origin-org/theme-ui`。它仍不拥有业务数据加载和跨领域流程。
...
```typescript
import { useThemeStorage } from "@origin-org/theme-sdk/storage";
...
数据默认落在 `~/.origin/desktop-app/themes/<themeId>/data.json`。当前主题卸载流程不会自动清理该目录。

## 包边界

| 包 | 应放内容 |
| --- | --- |
| `@origin-org/theme-sdk` | 主题协议、registry、host facade、model hook 和 props 类型 |
| `@origin-org/theme-ui` | 不绑定 Desktop 私有状态的表面、装饰和布局 primitive |
