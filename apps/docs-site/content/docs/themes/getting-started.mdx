...
当前流程适用于 Origin monorepo 中的内置或策展主题。以 `packages/themes/builtin/xianxia` 为可运行参考，并保持入口文件只负责组装主题能力。
...
<Steps>
  <Step>
    ...
    Vite 需要暴露 `./theme`，生成 `mf-manifest.json`，并将 React、`@origin-org/theme-sdk` 与宿主 UI 包配置为不重复打包的共享单例。
  </Step>
  <Step>
    ### 接入桌面应用
