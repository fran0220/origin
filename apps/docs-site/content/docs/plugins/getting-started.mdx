---
title: 创建第一个插件
description: 创建、构建并安装一个最小 Origin 桌面插件。
---

## 前置条件

- Node 或 Bun。
- 用于安装和调试插件的 Origin 桌面客户端（安装与热更新都要求它正在运行）。
...
<Callout title="给 Agent 的第一步" type="info">
  开发手册随 `@origin-org/plugin-sdk` 装进 `node_modules`，**不要硬编码那个路径**——工作区可能
...
npm run install:origin  # 构建、打包并装进正在运行的 Origin
npx origin-plugin-cli watch      # 热更新：宿主改从本工程目录加载，改完即生效
npx origin-plugin-cli reload <id>  # 提示有 pending 版本时切换过去
npx origin-plugin-cli uninstall  # 卸载
```

`install:origin` 把归档交给正在运行的桌面端校验、授权、安装，**不会**直接写 `~/.origin/plugins`。
...
通过 `@origin-org/plugin-vite` 配置 Module Federation：
...
<Tabs items={["开发调试", "发布安装"]}>
  <Tab value="开发调试">
    使用 `bunx origin-plugin dev` 启动插件开发服务，再通过 Origin 的开发链接加载。改动后可快速验证界面与工具注册，但发布前仍要执行生产构建并测试 ZIP 安装流程。
  </Tab>
  <Tab value="发布安装">
    <Steps>
      <Step>
...
它生成 `.origin/marketplace.json` 索引、`abilities/` 目录约定、一份仓库级 `AGENTS.md`，以及跑
...
```bash
npx @origin-org/plugin-cli sync          # 回填派生字段，并推进 marketplaceVersion
