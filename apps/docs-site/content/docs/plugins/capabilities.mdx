...
插件 API 由 `@origin-org/plugin-sdk` 提供，构建由 `@origin-org/plugin-vite` 提供，脚手架与安装命令由 `@origin-org/plugin-cli` 提供。插件代码运行在 Desktop renderer 进程的共享 JavaScript realm 内；权限声明用于知情同意和宿主 API 门控，不是 iframe、Worker 或恶意代码沙箱。
...
| 网页录制 | `ctx.recording` | `recording:capture` | 旧宿主上为 `undefined`，使用前判空；需要 Plugin API `^2.7.0` |
...
| 写入评估标准 | `ctx.evaluation.upsertDefinition` | `evaluation:write` | 不改写 Attempt；需要 Plugin API `^2.8.0` |
| 解析项目能力键 | `ctx.project.resolve` | `workspace.read` | 插件不得自己 hash cwd；需要 Plugin API `^2.8.0` |
...
| 贡献智能体 | `plugin.json` 的 `agent.agents` | 无 | 宿主铺成普通档案，作为 Thread 人格；不要写 `agent.teams` |
...
<Callout title="有两个字段必须判空" type="warn">
  `ctx.capture` 与 `ctx.recording` 在不支持离屏截图/录制的旧宿主上是 `undefined`；`ctx.gateway` 只对内置官方插件可用，第三方插件一律读到 `undefined`。使用前先判空，不要假设它们存在。
</Callout>
