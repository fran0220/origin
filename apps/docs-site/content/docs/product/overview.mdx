...
Origin 桌面端将 Agent、项目上下文和本机工具整合在一个工作空间中。以下按「要完成什么」组织，不要求先理解内部架构。

第一次使用时，先阅读[理解 Origin 的工作方式](/core/overview/)；它解释工作区、会话、工具和结果之间的关系，避免把 Origin 当成只生成答案的聊天框。
...
<Entries>
  ...
  <Entry href="/product/remote-control/" kicker="06 / PREVIEW" title="远程连接">
    从 Android 查看 Windows Desktop，并按授权执行远程输入。
  </Entry>
...
<Continue>
  ...
  <ContinueLink href="/product/agent-profiles/" title="Thread 协作" description="用独立 Thread 并行做事，而不是组一支常驻团队。" />
  <ContinueLink href="/product/design/" title="用 Origin 做设计" description="在设计画布上生成真实界面，选中改稿、预览与导出。" />
  ...
  <ContinueLink href="/product/remote-control/" title="远程连接与移动端" description="了解配对、屏幕查看、远程输入和恢复路径。" />
