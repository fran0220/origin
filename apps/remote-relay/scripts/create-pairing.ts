...
console.info(`ORIGIN_REMOTE_CONTROL_URL=${websocketBase}/v1/relay/${pairingId}/desktop`);
console.info(`ORIGIN_REMOTE_PAIRING_TOKEN=${token}`);
console.info(`Mobile target: ${websocketBase}/v1/relay/${pairingId}/mobile#${token}`);
console.info("Optional input grant: ORIGIN_REMOTE_DESKTOP_INPUT_ENABLED=true");
