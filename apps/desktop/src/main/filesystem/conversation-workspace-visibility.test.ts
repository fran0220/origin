...
/**
 * DEFAULT_CONVERSATION_CWD 在模块加载时由 ORIGIN_HOME 派生，因此本文件不做静态导入：
 * 先把 ORIGIN_HOME 指到临时目录，再动态加载被测模块，让「对话」根落在可控位置。
 */
describe("「对话」根目录下会话工作区的列举可见性", () => {
	...
	beforeAll(async () => {
		originHome = await mkdtemp(join(tmpdir(), "origin-conv-visibility-"));
		previousVettaHome = process.env.ORIGIN_HOME;
		process.env.ORIGIN_HOME = originHome;
		vi.resetModules();
	...
	afterAll(async () => {
		if (previousVettaHome === undefined) {
			delete process.env.ORIGIN_HOME;
		} else {
			process.env.ORIGIN_HOME = previousVettaHome;
		}
		vi.resetModules();
		if (originHome) await rm(originHome, { recursive: true, force: true });
	});
