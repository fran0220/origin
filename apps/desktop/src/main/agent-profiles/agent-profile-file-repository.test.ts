...
import { createAgentProfileStorageKey, readAgentProfileStorageIndex } from "./agent-profile-storage-layout.js";
...
async function createRepository(): Promise<{
	readonly repository: ReturnType<typeof createAgentProfileFileRepository>;
	readonly root: string;
}> {
	const root = await mkdtemp(join(tmpdir(), "origin-agent-profiles-"));
	temporaryDirectories.push(root);
	return { repository: createAgentProfileFileRepository({ root }), root };
}

function storedAgentRoot(root: string, agent: { readonly id: string; readonly name: string }): string {
	return join(root, "agents", createAgentProfileStorageKey(agent.name, agent.id));
}
...
describe("Agent Profile file repository", () => {
	it("writes metadata and long descriptions as separate files and reloads them", async () => {
		...
		const index = await readAgentProfileStorageIndex(root);
		expect(index.agents[firstAgent.id]).toBe(createAgentProfileStorageKey(firstAgent.name, firstAgent.id));
	});
