...
import { parseAgentProfileDocument } from "@origin/agent-profile";
...
import {
	type AgentProfileStorageIndex,
	agentDefinitionPath,
	agentProfileAgentsRoot,
	createAgentProfileStorageKey,
	readOptionalAgentProfileStorageIndex,
} from "./agent-profile-storage-layout.js";
import { reconcilePluginAgentPresets } from "./plugin-agent-preset-reconcile.js";

const log = getAppLogger("agent-profiles");

const TEAMS_DIR = join(getOriginHomePath(), "agent-profiles");
const INITIALIZED_MARKER = ".initialized";
const INDEX_FILE = "index.json";

export interface AgentProfileFileRepository {
	read(): Promise<AgentProfileDocument>;
	write(document: AgentProfileDocument): Promise<void>;
}

export interface AgentProfileFileRepositoryOptions {
	readonly root?: string;
}

/** A directory-backed repository. JSON stores indexes; long descriptions live in Markdown files. */
export function createAgentProfileFileRepository(
	options: AgentProfileFileRepositoryOptions = {},
): AgentProfileFileRepository {
	return new DirectoryAgentProfileRepository(options.root ?? TEAMS_DIR);
}

class DirectoryAgentProfileRepository implements AgentProfileFileRepository {
	/**
	 * 上一次读取中读坏的 Agent 目录。写回时必须保留它们：
	 * 读不出来只说明这一份数据坏了，不代表用户删除了这个 Agent。
	 */
	private unreadableAgentIds: ReadonlySet<string> = new Set();
	private storageIndex: AgentProfileStorageIndex | undefined;

	constructor(private readonly root: string) {}

	async read(): Promise<AgentProfileDocument> {
		await mkdir(this.root, { recursive: true });
		const index = await readOptionalAgentProfileStorageIndex(this.root);
	...
	private async installPresets(
		...
		index: AgentProfileStorageIndex,
	...
	async write(document: AgentProfileDocument): Promise<void> {
		await mkdir(this.root, { recursive: true });
		const currentIndex = this.storageIndex ?? (await readOptionalAgentProfileStorageIndex(this.root));
		const agentDirectories: Record<string, string> = {};
		for (const agent of document.agents) {
			const directory = currentIndex.agents[agent.id] ?? createAgentProfileStorageKey(agent.name, agent.id);
		...
		await removeDeletedMappedDirectories(agentProfileAgentsRoot(this.root), currentIndex.agents, agentDirectories);

		const nextIndex: AgentProfileStorageIndex = {
	 ...
	 */
	private async readAgents(index: AgentProfileStorageIndex): Promise<AgentProfile[]> {
