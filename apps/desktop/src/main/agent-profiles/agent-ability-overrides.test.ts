...
describe("agent ability overrides", () => {
	...
	it("keeps inheriting in `all` mode even with pinned plugins", () => {
		...
		expect(toAgentConfigurationOverrides(ALL, { plugins: ["origin-ui-design"] })).toEqual({});
	});

	it("unions pinned plugins into an explicit selection", () => {
		expect(toAgentConfigurationOverrides(CUSTOM, { plugins: ["origin-ui-design"] })).toEqual({
			skills: ["a"],
			mcpServers: ["b"],
			plugins: ["c", "origin-ui-design"],
		});
	});
