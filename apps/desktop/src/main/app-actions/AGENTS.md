...
- 插件 activation 使用 `begin → stage → commit/abort`：commit 会先完整校验 staging，再以 provider 快照原子替换旧 activation；失败时旧版本继续服务，不出现空目录窗口。
...
- 插件热更新 activation 失败时，旧版本仍可 search/describe/run；成功时同一时刻切换整组 Action。
