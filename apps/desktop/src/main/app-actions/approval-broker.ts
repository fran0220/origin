...
export class ActionApprovalBroker implements ActionApprovalRequester {
	...
	request(request: ActionApprovalRequest, signal?: AbortSignal): Promise<ActionApprovalDecision> {
		if (this.webContents.isDestroyed()) {
			...
			return Promise.reject(new ActionError("ACTION_APPROVAL_UNAVAILABLE", "Origin Desktop 授权界面不可用。"));
		}
		...
		return new Promise<ActionApprovalDecision>((resolve, reject) => {
			...
			const onAbort = (): void => {
				...
				cancel(new ActionError("ACTION_CANCELLED", "Origin action 请求已取消。", { actionId: request.actionId }));
			};
	...
	cancelAll(): void {
		...
		for (const pending of this.pending.values()) {
			pending.cancel(new ActionError("ACTION_CANCELLED", "Origin Desktop 授权请求已取消。"));
		}
		this.pending.clear();
	}
