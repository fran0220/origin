...
const APP_BUNDLE_NAME = "Origin Computer Use.app";
const EXECUTABLE_NAME = "Origin Computer Use";

/**
 * Resolve the absolute path to the `Origin Computer Use.app` bundle (macOS only).
 *
 * In packaged builds the bundle lives under
 *   <Resources>/appshot/Origin Computer Use.app
 * (staged by scripts/prepare-pack.js via extraResources).
 *
 * In dev mode (`!app.isPackaged`) we fall back to
 *   <apps/desktop>/resources/appshot/bin/Origin Computer Use.app
