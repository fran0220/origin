...
const HELP_TEXT = `Origin OCR command line interface

Usage:
  Origin.exe --ocr-pdf <input.pdf>   --output <output.json> [pdf options]
  Origin.exe --ocr-img <input.image> --output <output.json> [img options]
  Origin.exe ocr pdf <input.pdf>     --output <output.json> [pdf options]
  Origin.exe ocr img <input.image>   --output <output.json> [img options]

Description:
  Run local OCR on a multi-page PDF using Origin Desktop's bundled Electron
