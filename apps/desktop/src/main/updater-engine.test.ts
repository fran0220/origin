...
describe("ElectronUpdaterEngine", () => {
	...
	it("marks the app as quitting before activating the Inno update", async () => {
		...
		const updateInfo: UpdateInfo = {
			version: "1.2.3",
			files: [{ url: "Origin-1.2.3-win-x64.exe", sha512: "hash", size: 42 }],
		...
		const innoUpdate = {
			select: vi.fn().mockReturnValue({ assetFileName: "Origin-1.2.3-win-x64.exe", totalBytes: 42 }),
	...
	it("passes provider-resolved asset URLs to the Inno updater", async () => {
		...
		const prepareDownloadedInstaller = vi.fn().mockResolvedValue(["C:\\staged\\Origin.exe"]);
		...
		const updater = {
			...
			downloadUpdate: vi.fn().mockResolvedValue(["C:\\pending\\Origin-1.2.3-win-x64.exe"]),
		...
		await expect(engine.downloadUpdate(vi.fn()).promise).resolves.toEqual(["C:\\staged\\Origin.exe"]);
		expect(prepareDownloadedInstaller).toHaveBeenCalledWith(
			"C:\\pending\\Origin-1.2.3-win-x64.exe",
			expect.any(Function),
