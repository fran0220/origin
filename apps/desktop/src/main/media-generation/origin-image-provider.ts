...
import {
	...
	getCloudBridge,
	type OriginGatewayRequest,
	type OriginGatewayResponse,
...
type GatewayRequest = <T>(request: OriginGatewayRequest, signal?: AbortSignal) => Promise<OriginGatewayResponse<T>>;
...
export function createOriginImageProvider(
	artifacts: MediaArtifactStore,
			...
			if (input.operation !== "generate") {
				return Promise.resolve({
					...
					error: {
						...
						message: `Origin image provider does not support ${input.operation}`,
						retryable: false,
					},
				});
			}
