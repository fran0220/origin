...
export function registerAbilitiesIpc(): () => void {
	...
	const unsubscribeFromUpdates = openMarketplace.subscribeToUpdates(() => {
		for (const win of BrowserWindow.getAllWindows()) {
			if (!win.isDestroyed()) win.webContents.send("origin:abilities:open-marketplaces-updated");
		}
	});
	// 一次性下发全量台账；读取时顺带剔除漂移条目（ADR-0049）。
	ipcMain.handle("origin:abilities:get-ledger", () => readAbilityLedger());
	ipcMain.handle("origin:abilities:list-local-presentations", () => listLocalAbilityPresentations());
	ipcMain.handle("origin:abilities:list-open-marketplace", async () => {
		const { source: _source, ...snapshot } = await openMarketplace.listSource(DEFAULT_MARKETPLACE_SOURCE_ID);
		return snapshot;
	});
	ipcMain.handle("origin:abilities:refresh-open-marketplace", async () => {
		const { source: _source, ...snapshot } = await openMarketplace.refreshSource(DEFAULT_MARKETPLACE_SOURCE_ID);
		return snapshot;
	});
	ipcMain.handle("origin:abilities:list-open-marketplaces", () => openMarketplace.list());
	ipcMain.handle("origin:abilities:refresh-open-marketplaces", () => openMarketplace.refresh());
	ipcMain.handle("origin:abilities:list-marketplace-sources", () => openMarketplace.listSources());
	ipcMain.handle("origin:abilities:add-marketplace-source", (_event, input: unknown) =>
		openMarketplace.addSource(parseAddSourceInput(input)),
	);
	ipcMain.handle("origin:abilities:update-marketplace-source", (_event, id: unknown, input: unknown) =>
		openMarketplace.updateSource(requireString(id, "id"), parseUpdateSourceInput(input)),
	);
	ipcMain.handle("origin:abilities:clear-marketplace-source-credential", (_event, id: unknown) =>
		openMarketplace.clearSourceCredential(requireString(id, "id")),
	);
	ipcMain.handle("origin:abilities:remove-marketplace-source", (_event, id: unknown) =>
		openMarketplace.removeSource(requireString(id, "id")),
	);
	ipcMain.handle("origin:abilities:refresh-marketplace-source", (_event, id: unknown) =>
		openMarketplace.refreshSource(requireString(id, "id")),
	);
	ipcMain.handle(
		"origin:abilities:install-open-ability",
	...
	ipcMain.handle("origin:abilities:prepare-open-mcp-ability", (event, slug: unknown, sourceId: unknown) => {
		const resolvedSourceId = sourceId === undefined ? undefined : requireString(sourceId, "sourceId");
		const progress = (payload: OpenMarketplaceMcpRuntimeProgress): void => {
			if (!event.sender.isDestroyed()) event.sender.send("origin:abilities:mcp-runtime-progress", payload);
		};
		return openMarketplace.prepareMcp(requireString(slug, "slug"), resolvedSourceId, progress);
	});
	ipcMain.handle("origin:abilities:get-open-mcp-setup-status", () => openMarketplace.mcpSetupStatus());
	ipcMain.handle("origin:abilities:remove-open-mcp-runtime", (_event, slug: unknown, sourceId: unknown) => {
	...
	ipcMain.handle(
		"origin:abilities:record-mcp-install",
	...
	return () => {
		...
		ipcMain.removeHandler("origin:abilities:get-ledger");
		ipcMain.removeHandler("origin:abilities:list-local-presentations");
		ipcMain.removeHandler("origin:abilities:record-mcp-install");
		ipcMain.removeHandler("origin:abilities:list-open-marketplace");
		ipcMain.removeHandler("origin:abilities:refresh-open-marketplace");
		ipcMain.removeHandler("origin:abilities:list-open-marketplaces");
		ipcMain.removeHandler("origin:abilities:refresh-open-marketplaces");
		ipcMain.removeHandler("origin:abilities:get-open-mcp-setup-status");
		ipcMain.removeHandler("origin:abilities:list-marketplace-sources");
		ipcMain.removeHandler("origin:abilities:add-marketplace-source");
		ipcMain.removeHandler("origin:abilities:update-marketplace-source");
		ipcMain.removeHandler("origin:abilities:clear-marketplace-source-credential");
		ipcMain.removeHandler("origin:abilities:remove-marketplace-source");
		ipcMain.removeHandler("origin:abilities:refresh-marketplace-source");
		ipcMain.removeHandler("origin:abilities:install-open-ability");
		ipcMain.removeHandler("origin:abilities:prepare-open-mcp-ability");
		ipcMain.removeHandler("origin:abilities:remove-open-mcp-runtime");
	};
}
