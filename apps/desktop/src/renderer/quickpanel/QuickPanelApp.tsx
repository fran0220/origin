...
export function QuickPanelApp(): JSX.Element {
	...
	const submitNew = useCallback(() => {
		...
		if (!text) return;
		void window.originQuickPanel?.createConversation(text);
	}, [input]);

	const openItem = useCallback((item: QuickPanelItem) => {
		void window.originQuickPanel?.openSession({ sessionPath: item.sessionPath, cwd: item.cwd });
	}, []);
