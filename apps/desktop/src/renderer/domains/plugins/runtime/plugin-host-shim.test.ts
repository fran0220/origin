...
/**
 * `origin-host://*` 的生成式 shim（主进程 plugin-protocol.ts）在插件模块求值时读
 * `globalThis.__ORIGIN_PLUGIN_HOST__.<key>`。这个全局对象漏一个 key 不会有任何类型
...
function hostKeysReadByProtocolShim(): string[] {
	...
	for (const match of source.matchAll(/__ORIGIN_PLUGIN_HOST__\.([A-Za-z_$][\w$]*)/g)) {
		keys.add(match[1]);
	}
	return [...keys].sort();
}
...
describe("installPluginHostShim", () => {
	it("装上 plugin-shared-modules 里的全部宿主模块", () => {
		...
		expect(globalThis.__ORIGIN_PLUGIN_HOST__).toBeDefined();
		for (const key of Object.keys(pluginHostShimModules)) {
			expect(globalThis.__ORIGIN_PLUGIN_HOST__?.[key as keyof typeof pluginHostShimModules]).toBeDefined();
		}
	});

	it("主进程 shim 读到的每个 key 都存在——漏一个就是插件静默消失", () => {
		installPluginHostShim();
		const host = globalThis.__ORIGIN_PLUGIN_HOST__;
		...
		for (const key of readKeys) {
			expect(
				host?.[key as keyof typeof pluginHostShimModules],
				`__ORIGIN_PLUGIN_HOST__.${key} 未装载`,
			).toBeDefined();
		}
	});
	...
	it("theme-ui 侧的模型选择器可达（0.3.0 看板加载失败的直接原因）", () => {
		installPluginHostShim();
		expect(globalThis.__ORIGIN_PLUGIN_HOST__?.themeUiPlugin.ModelSelectorView).toBeDefined();
	});
});
