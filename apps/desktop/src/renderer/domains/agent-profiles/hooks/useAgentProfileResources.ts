...
import { loadAgentProfileConfigurationResources } from "../services/load-agent-profile-resources";
...
export function useAgentProfileResources(): AgentProfileResources {
	...
	useEffect(() => {
		...
		void waitForCommittedPaint()
			.then(() => {
				if (cancelled) return;
				return loadAgentProfileConfigurationResources();
			})
			.then((resources) => {
			...
			.catch((cause: unknown) => {
				if (!cancelled) setError(agentProfileErrorMessage(cause));
			})
			.finally(() => {
	 ...
	 */
	useEffect(() => {
		return window.originApp.agentProfiles.onChanged(() => {
			void loadAgentProfileConfigurationResources()
				.then((resources) => {
					setDocument(resources.document);
					setBlueprints(resources.blueprints);
					setPlugins(resources.plugins);
	...
	const displayDocument = useLocalizedAgentProfileDocument(document);
...
export function agentProfileErrorMessage(cause: unknown): string {
