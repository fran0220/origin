import type { SkillPresentation, SkillPresentationSurface, SkillVisibility } from "@origin-org/capability-sdk";
...
export interface DesktopSkillsApi {
	/**
	 * 列出可调用资源；未登记清单、直接放入用户 Origin 配置目录下 `scene` 的合法场景也会以 `source="scene"` 返回。
	 * @param cwd 当前会话/项目 cwd，用于发现项目级 `<cwd>/.agents/skills` 与 `<cwd>/.origin/skills`；省略则仅列全局来源。
	 */
	...
	/** 安装类型由包内 SKILL.md 的 `metadata.type` 决定（scene 装进 `~/.origin/scene/`）。 */
