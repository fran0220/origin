...
test("packaged E2E update feed serves a checksum-bearing downloadable fixture", async () => {
	...
	const artifact = await (await fetch(`${fixture.url}Origin-e2e-update.AppImage`)).arrayBuffer();

	assert.match(metadata, /version: 0\.5\.47/);
	assert.equal(Buffer.from(artifact).toString("utf8"), "origin-packaged-e2e-update\n");
});
