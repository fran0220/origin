...
-- 作用：对用户安装到 /Applications 或 ~/Applications 的 Origin.app
-- 执行 xattr -dr com.apple.quarantine，绕过 Gatekeeper 的「应用程序已损坏」拦截。

on findVettaApp()
	set homeApps to (POSIX path of (path to home folder)) & "Applications/Origin.app"
	set candidatePaths to {"/Applications/Origin.app", homeApps}
...
if vettaPath is missing value then
	display dialog "未检测到 Origin.app。请先将 DMG 中的 Origin.app 拖入 Applications 文件夹，然后再次打开「修复已损坏」。" buttons {"好"} default button "好" with icon caution with title "修复已损坏"
	return
end if
