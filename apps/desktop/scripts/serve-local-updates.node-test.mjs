...
test("serves byte ranges as 206 with the correct slice", async () => {
	...
	const partial = await fetch(`${base}/Origin-1.2.3-arm64-mac.zip`, { headers: { Range: "bytes=4-8" } });
	...
	const whole = await fetch(`${base}/Origin-1.2.3-arm64-mac.zip`);
...
test("rejects unsatisfiable ranges and directory traversal", async () => {
	...
	const unsatisfiable = await fetch(`${base}/Origin-1.2.3-arm64-mac.zip`, { headers: { Range: "bytes=99-200" } });
