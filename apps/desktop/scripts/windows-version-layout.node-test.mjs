...
test("createWindowsVersionLayout keeps launcher, pointer, and NSIS compatibility directory at root", async () => {
	...
	const launcherPath = join(directory, "OriginLauncher.exe");
	await writeFile(launcherPath, "launcher");

	await createWindowsVersionLayout(directory, "1.2.3", launcherPath);

	assert.equal(await readFile(join(directory, "Origin.exe"), "utf8"), "launcher");
	assert.equal(await readFile(join(directory, "versions", "1.2.3", "Origin.exe"), "utf8"), "electron");
