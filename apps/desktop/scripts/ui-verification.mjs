...
async function inspectCdp(endpoint) {
	try {
		...
		const mainWindow =
			pages.find((target) => target.title === "Origin Desktop") ??
...
function selectMainWindow(layout, uiInfo) {
	...
	const matchingLine = tabListResult.stdout
		.split(/\r?\n/)
		.find((line) => line.includes("[Origin Desktop](") && (!mainWindowUrl || line.includes(mainWindowUrl)));
	const index = matchingLine?.match(/^- (\d+):/)?.[1];
	if (index === undefined) throw new Error("Unable to find the Origin Desktop renderer tab");
	const selectResult = runPlaywright(layout, ["tab-select", index]);
	if (selectResult.status !== 0) throw new Error("Unable to select the Origin Desktop renderer tab");
}
