...
test("contentTypeFor publishes native package formats with package media types", () => {
	...
	assert.equal(contentTypeFor("Origin-1.2.3-win-x64.msi"), "application/x-msi");
});
	...
	try {
		await Promise.all([
			writeFile(
				...
				"version: 1.2.3\nfiles:\n  - url: Vetta%20Setup%201.2.3.exe\npath: Origin Setup 1.2.3.exe\n",
			),
			writeFile(join(directory, "Origin Setup 1.2.3.exe"), "installer"),
			writeFile(join(directory, "Origin Setup 1.2.3.exe.blockmap"), "blockmap"),
			writeFile(join(directory, "Origin-1.2.3-win-x64.msi"), "msi"),
			writeFile(join(directory, "Origin-1.2.3-win-x64.zip"), "zip"),
			writeFile(join(directory, "Origin Setup 1.2.2.exe"), "stale"),
			writeFile(join(directory, "Origin-1.2.2-win-x64.msi"), "stale"),
		]);

		assert.deepEqual(await collectArtifacts(directory), [
			"Origin Setup 1.2.3.exe",
			"Origin Setup 1.2.3.exe.blockmap",
