...
- 自由新增主题自己的 UI。
- 重排默认组件。
- 复用 `@origin-org/theme-ui` 或桌面端公开 UI 出口中的 public primitives。
...
desktop 可以额外提供窄口径的官方 UI primitive 出口，例如 `@origin/desktop-theme-ui/sidebar`。这类出口只应导出稳定的 props-driven 子组件和类型，例如 `SidebarNavigation`、`SidebarNavItemButton`、`SidebarNavigationProps`、`SidebarNavItem`；不要从该出口导出完整 sidebar connected container 或会牵出整个领域依赖树的组件。
