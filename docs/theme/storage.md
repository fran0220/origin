...
主题可以在用户本地持久化**自己的**小型 JSON 数据（进度、偏好、页面状态），但不能直接访问 `localStorage`、`window.originApp` 或文件系统。
...
- 直接调用 `window.originApp.themes.storage`（该 API 仅供 host 实现使用）。
...
```txt
~/.origin/desktop-app/themes/<themeId>/data.json
```
...
注意：这与远程主题包安装目录 `~/.origin/themes/` 分离，避免包资源与用户数据混放。
...
| 多窗口 | main 广播 `origin:themes:storage:changed`，各窗口缓存对齐 |
| 切换主题 | 不删除旧主题数据；切回可恢复 |
...
Theme component
  → useThemeStorageValue / useThemeStorage   (sdk facade)
    → ThemeHost.storage.useThemeStorage()    (desktop)
      → 内存 cache
      → window.originApp.themes.storage.*        (preload, host only)
        → main theme-data-store
          → ~/.origin/desktop-app/themes/<id>/data.json
```
...
- 落盘：`~/.origin/desktop-app/themes/xianxia/data.json`
