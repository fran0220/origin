...
| `agent.agents` | **插件贡献的智能体**：人设、头像、系统提示词由插件提供，宿主把它们铺进用户的智能体库。**不需要权限**，见[贡献智能体](#贡献智能体)。 |
...
`agent.agents` 让插件把**自己的人设**带进产品：宿主在插件启用时把它们铺进用户的智能体库，与用户自建的档案并列出现在智能体中心、新会话选择器和 `@` 提及里。它们是 Thread 的可选人格，不再组成持久团队。

**这些资产由你 1:1 维护**：清单是唯一的真相源，宿主每次同步都按它重铺一遍——你改名、改任务书、撤掉一个智能体，用户升级后都会如实生效。对应地，用户改不动也删不掉它们（UI 只读展示）；想要一份自己调教的版本，用户复制一份自建档案即可。
...
智能体可通过 `roles` 声明自己能顶的角色，供其他插件或用户档案引用。插件不再贡献团队；清单里不要写 `agent.teams`。
...
下面这些由预置插件供货，**任何一台机器上都解析得到**。`role` 是智能体声明的角色 slug；想钉死某一个人，用 `agent` 列的全名。
...
角色词表**不是白名单**：写表外的角色照样能声明。你的插件也可以给自己的智能体写上这几个 role，用户装了之后同一个角色就多一个候选（本插件优先，其次按 `pluginId` 字典序）。

### 生命周期

- **启用插件**：宿主把缺失的档案补齐——判据是「用户文档里现在有没有」，不是「历史上铺过没有」。因此旧版本数据缺失、一次异常都会被补回来。
- **升级插件**：清单里现在写的就是用户拿到的：名称、说明、能力按新清单整体重铺（档案里**不落 `systemPrompt`**，人设升级同样自动生效）。档案 id 不变，用户的会话绑定因此不受影响。
- **撤掉资产**：新版本不再声明的智能体会从用户那里消失。这是「作者收得回自己发出去的东西」的那一条，与升级走的是同一次同步。
- **禁用插件**：档案**灰着留在原地**并标出「插件已禁用」。禁用不等于撤掉：清单还声明着它，就不会被当成残骸清理。重新启用后一切原样回来。
- **热重载**：开发态改完 manifest 重载插件，智能体当场更新，不需要重启 App。
- **贡献出错**：单个智能体解析失败（提示词读不到、头像超限）只跳过它自己并打 warn，不影响同插件的其它贡献。

### 接管与升级（legacyIds）

`legacyIds` 用于「人设从别处迁进插件」：宿主解析不到这些历史 id 时折算到本智能体，铺档案时也据此**认领**用户已有的同角色档案，而不是再铺一份新的。认领只保留**身份**（档案 id），内容按你的清单重铺。
