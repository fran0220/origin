...
新增 `scripts/quality/check-legacy-execution-retirement.mjs`，使用 TypeScript AST 扫描 `coding-agent`、`cli-host` 和 `desktop` 的生产源码，并冻结当前 15 条 Legacy 执行或暴露边：
...
- `apps/desktop/src/main/greenfield-runtime/desktop-legacy-session-migration-backend.ts`
