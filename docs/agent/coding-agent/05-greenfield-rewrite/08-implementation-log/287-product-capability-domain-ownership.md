...
旧 `greenfield-mcp-deferred-adapter.ts` 的生产调用者为零，并重复实现了 `@origin/runtime-mcp` 已提供的 Deferred Controller、Tool Search 和提示词行为。本轮直接删除该文件，没有保留转发、别名或兼容包装；现有 MCP Session Coordinator 继续使用正式 `McpDeferredToolController`。
