...
- `@origin/ai` 和 `@origin/agent-core` 作为已经独立的下层能力继续使用，除非单独审计发现其合同无法满足新内核。
...
4. `@origin/ai` 继续负责模型供应商协议和统一消息格式。
5. `@origin/agent-core` 继续负责单轮模型调用、工具循环和基础事件，重写前先通过合同测试验证，而不是默认重写它。
...
仓库已经有 `@origin-org/capability-sdk` 和 `@origin/capability-runtime`。它们定义的是：
