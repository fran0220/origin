...
`@origin/coding-agent/host` 作为组合层稳定入口提供；
...
同时审计了 `@origin/runtime-tools` 包根：仓库内源码和测试当前没有直接消费者，但包根仍是已
...
110 阶段重新验收时修复了 Desktop Vitest 未将 `@origin/runtime-composition` 指向源码的问题，并将全局 Agent
...
产品实现现已迁入 `@origin/coding-agent/composition`：
...
第 116 轮没有收缩 `@origin/coding-agent` 已发布根入口，而是先把仓库内消费者从“从一个聚合根取得所有能力”
...
`@origin/coding-agent` 根入口引用，结构守卫因此删除全部路径允许项；测试仍可使用根入口验证兼容性。
...
| Composition Root 与依赖图 | 产品装配已归属 `@origin/coding-agent/composition`；CLI/Desktop 直接消费；`runtime-composition` 无包装兼容转发；Runtime 子路径与旧根兼容面采用分段构建；manifest truth 和 forwarding-only 守卫已接入 | `runtime-tools`、`runtime-storage` 包根仍需为外部消费者保留 Coding Agent 兼容转发；默认 selector 仍是 Legacy | 产品所有权和 clean build 顺序已收口；兼容根入口只能在外部迁移窗口后删除 |
