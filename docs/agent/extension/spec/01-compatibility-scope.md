...
| 状态 | 定义 | 用户可见行为 |
| --- | --- | --- |
| `lossless` | Pi 语义与 Origin 合同等价 | 正常加载，无兼容警告 |
| `adapted` | 通过明确转换保持主要行为 | 报告转换点和已知差异 |
| `host-dependent` | 仅某些宿主有等价能力 | 在支持宿主启用，其余宿主报告 inapplicable |
| `excluded` | 明确不属于 Origin 兼容目标 | 不进入路线；可安全剥离时记录诊断，否则拒绝加载 |
...
1. Origin 已有或适合新增宿主中立的 public port，不需要暴露 Pi manager/component；
...
满足产品价值但 Origin 当前合同尚不完整的能力，必须先按 [Origin 原生能力先行方案](07-vetta-native-first.md) 实现。Pi adapter 只能映射已经通过 native Extension fixture 的能力，不能在 `pi-compat` 内补一套临时目录、事件或执行器。
...
其中 Tool 执行、共有事件、Command 和大部分 actions 已有原生基础；catalog/generation、Tool input normalize/custom validator、Tool prompt contribution、状态变化事件和 Provider owner/unregister 是兼容前要先补齐的 Origin 原生能力。

### L0：模块与 Package 发现

| 功能 | 状态目标 | 说明 |
| --- | --- | --- |
| `package.json#pi` 的 extensions/skills/prompts | `lossless/adapted` | 复用 Origin 已有发现与来源模型 |
| `package.json#pi` 的 themes | `excluded` | Pi Theme 属于 TUI 展示资源，不安装为 Origin Theme |
| `@earendil-works/pi-coding-agent` | `adapted` | 指向专用 facade，不指向 Origin 包根 |
...
| 历史 `@sinclair/typebox` imports | `adapted` | Pi loader 的兼容行为，不能污染 Origin native authoring contract |
| Pi AI/Agent 包 | 分 export 判定 | 只提供行为子集需要的 facade exports，不导出整个 Origin 内部实现 |
| `@earendil-works/pi-tui` / legacy namespace | `excluded` | 不提供 virtual module；存在 runtime import 时拒绝加载 |

### L1：首个可用版本

优先覆盖 Pi 生态中最常见、且可由 Origin 宿主无关合同表达的能力：
...
| `executionMode` | 部分 `adapted` | `undefined/sequential` 匹配 Origin 当前语义；`parallel` unsupported |
...
| model/thinking getters/setters | `adapted` | 使用 Origin model catalog 和公开 thinking contract |
| `exec` | `adapted` | 使用 Origin 现有执行 action，保留 cwd/signal/error 语义 |
| Extension event bus | `adapted` | subscription 归属 generation，reload 自动取消 |
| `notify/select/confirm/input` | `host-dependent` | 映射为 Origin 结构化交互；不传递 Pi Component/Theme |
...
| `newSession/fork/switchSession` | `adapted` | 使用 Origin transition transaction |
| `withSession` | `adapted` | commit 后提供绑定新 session 的 fresh context |
| `navigateTree/reload/compact` | `adapted` | 复用现有 Session Host，保持取消与失败回滚 |
| `ctx.sessionManager` | 部分 `adapted` | 提供只读 Pi facade；不暴露 Origin manager |
| `ctx.modelRegistry` | 部分 `adapted` | 提供只读查询和 auth facade；写操作走专用 Provider port |
| `agent_settled/session_info_changed/thinking_level_select` | 部分 `adapted` | 先新增 Origin native fact events，再投影可表达字段；不用近似事件伪造 |
...
| 能力 | 首个 profile 状态 | 处理 |
| --- | --- | --- |
| config-form `registerProvider(name, config)` 字段交集 | `adapted` | 只接受 Origin ProviderConfig 可表达的 allowlist |
...
| full native `Provider` | `unsupported` | 不把 Pi Provider 对象接入 Origin Runtime |
...
Provider 不能为了提高表面兼容率而直接把 Pi `Provider` cast 成 Origin Provider。两边的消息、usage、stop、auth、refresh 和 stream 回调已经分叉。
...
| `Component`、Theme、Keybindings、raw terminal input | `excluded` | 具体终端对象不是 Origin 行为合同 |
...
`notify/select/confirm/input` 不属于上述 TUI 兼容：facade 只传递字符串、选项、结果、取消和 timeout，将其转换为 Origin 自己的结构化交互。任何需要 Pi `Component`、Theme 或 terminal redraw 的调用都不支持。
...
- 不直接依赖 `@earendil-works/pi-coding-agent` 作为 Origin 生产运行时。
- 不承诺 Pi 未声明的私有深度导入。
