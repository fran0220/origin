...
Pi 兼容层不得成为新能力的事实源。一个 Pi 能力如果对 Origin 本身有产品价值，应先形成 Origin 原生 Extension/Runtime 合同、通过 native tests，再由 `pi-compat` 做字段和事件投影。只有包名、TypeBox 1 authoring 和 Pi event shape 等第三方差异留在 Anti-Corruption Layer。

```text
先证明 Origin 需要该能力
  -> 在正确 owner 中建立原生合同
  -> Origin native Extension 使用并通过合同测试
  -> Pi adapter 映射到该合同
  -> Pi differential fixture 验证映射
```

如果某项能力没有合适的 Origin native owner，就不应先在 Pi facade 中模拟它。这条规则可以避免形成“native Extension 一套语义、Pi Extension 另一套语义”。

## 当前基础与缺口

| 领域 | Origin 已有基础 | 当前缺口 | 结论 |
| --- | --- | --- | --- |
...
| Tool 输入 | Agent engine 已支持 `validateInput`；Origin native Tool 使用 TypeBox 0.34 | `runtime-core` 的 `RuntimeToolDefinition` 没有透传自定义 validator；无标准 normalize-before-validate hook | 补通用 validator port，再增加 native input normalization |
...
| Event | 已有大部分 session/agent/turn/message/tool 事件与 transform dispatcher | 缺少稳定的 settled、session metadata change、thinking level change 事件 | 只新增对 Origin 宿主也有价值的原生事件 |
...
| 结构化交互 | Origin 已有 `select/confirm/input/notify` contract | 原合同混有大量 terminal component 方法 | compat 只投影四个交互方法；不把 TUI 扩展当原生增强目标 |
...
所有 Origin native registration 先进入 `ContributionDraft`，成功后原子发布不可变 `ContributionSet`。这一步不增加 Pi API，先替换当前多个 Map 的所有权模型。
...
Native-first 是兼容演进，不是删除 Origin 现有能力。现有 Extension 的 Tool Schema、renderer、shortcut、command、flag、事件顺序和冲突策略先建立基线；必要时由 native-only contribution 保留。Pi profile 的 excluded 规则只限制 Pi adapter，不能反向降低 Origin native Extension。

### N2：Native Tool Contract

#### 输入 normalize 与 validator port

建议给产品 Extension Tool 增加命名为 Origin 语义的可选 hook，而不是复制 Pi 字段名：
...
- `executionMode: "parallel"`：Origin 当前 Tool batch 是 sequential，新增并发会改变结果顺序、steering、权限和取消语义。Pi 的 `undefined/sequential` 可兼容，`parallel` 报 unsupported。
...
新增事件使用 Origin 领域名称，不复制 Pi 名称：

| Origin native event | 建议语义 | Pi 投影 |
| --- | --- | --- |
...
| Pi-only deep exports | 没有 Origin native owner，也没有稳定行为合同 |
...
- 每个新能力至少有一个 Origin native Extension fixture，不依赖 Pi package；
...
- 删除 `pi-compat` 后，新增的 Origin 原生能力和测试仍全部成立；
