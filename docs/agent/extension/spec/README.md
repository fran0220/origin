...
目标是在 Origin 中运行只依赖双方共有行为合同的 Pi Extension，并消费 Pi Package 中适合 Origin 的资源；目标不是完整适配 Pi Extension API，不复制 Pi 的 `coding-agent/src/core`，也不兼容 Pi TUI。
...
Pi source module
  ...
  -> atomic publish
  -> existing Origin Extension/Runtime ports
```

但实施顺序是 **Origin native-first**：先把 catalog/generation、Tool 输入与 prompt、生命周期事件、Provider ownership 和结构化交互做成 Origin 原生能力，再接入 Pi facade。Pi 兼容层不得拥有 Origin native 尚不存在的 Tool 调度、事件或 Provider 生命周期。

核心约束：

- 只有一条 Agent、Tool 和 Session 执行路径；Pi Extension 最终进入 Origin 现有 Runtime。
- Pi 与 Origin 的作者 API 都编译成同一套 canonical contribution，不在 Runner 中长期维护两组分支。
- 每个新增能力先由 Origin native Extension fixture 验证；删除 `pi-compat` 后这些能力仍应完整成立。
...
- TypeBox 1 只作为 Pi authoring/runtime facade；跨边界归一化成带 dialect 的 plain JSON Schema，Origin native TypeBox 0.34 暂不迁移。
...
3. Pi 的 `SessionManager`、`ModelRegistry` 等具体类型不应进入 Origin 稳定领域合同；Pi TUI 类型不进入 facade。
...
2. [Origin 原生能力先行方案](07-vetta-native-first.md)
