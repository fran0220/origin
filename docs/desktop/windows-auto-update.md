...
本文记录 Origin Desktop Windows 自动更新从服务端发版迁移到 R2/GitHub Releases 的最终方案、实现细节、完整验证流程，以及开发期间遇到的问题。它是当前 Windows 更新链路的维护手册。
...
desktop/
  stable/
    latest.yml
    Origin-<version>-win-x64.exe
    Origin-<version>-win-x64.exe.blockmap
  test/
    latest.yml
    Origin-<version>-win-x64.exe
    Origin-<version>-win-x64.exe.blockmap
...
应用 ready
  ...
  -> 提升 EXE 为下一次差分基线
  -> Inno 静默展开到 %LOCALAPPDATA%\Origin\versions\<new>
  -> 校验 Origin.exe、resources/app.asar、.install-complete
  -> 状态变为 ready，提示更新并重启
...
1. 原子写入 `%LOCALAPPDATA%\Origin\current.json`，记录新旧版本并设 `pending=true`。
2. 使用 `app.relaunch({ execPath: <new Origin.exe> })` 启动新版本。
3. 退出旧版本。
...
apps/desktop/release/
  Origin-0.5.57-win-x64.exe
  Origin-0.5.57-win-x64.exe.blockmap
  Origin-0.5.57-win-x64.exe.files.json
  latest.yml
```
...
3. 验证 `Origin.exe`、`resources/app.asar` 和 `.install-complete`。
...
```powershell
Get-Content "$env:LOCALAPPDATA\Origin\current.json"
Get-ChildItem "$env:LOCALAPPDATA\Origin\versions"
...
- 运行进程来自 `%LOCALAPPDATA%\Origin\versions\<new>\Origin.exe`。
- 版本目录只保留新版本和上一版本。
...
```powershell
curl.exe -I "https://releases.openvetta.com/desktop/test/Origin-<version>-win-x64.exe"
curl.exe -r 0-1023 -o NUL -D - "https://releases.openvetta.com/desktop/test/Origin-<version>-win-x64.exe"
```
...
```powershell
curl.exe -H "Range: bytes=0-9,100-109" -o NUL -D - "https://releases.openvetta.com/desktop/test/Origin-<version>-win-x64.exe"
```
...
**修复**：后台更新不再卸载或覆盖当前安装目录，而是写入 `%LOCALAPPDATA%\Origin\versions\<new>`；后台 Inno 设置 `Uninstallable=no`、不创建卸载注册项。

如果手动执行修复安装仍遇到该提示，应完全退出 Origin（包括托盘和子进程）后重试。不要把手动修复安装当作正常自动更新步骤。
...
1. `%LOCALAPPDATA%\Origin\current.json` 没有写成新版本：激活动作没有完成，检查主进程的 `install failed`。
2. 指针已更新但没有新版本进程：检查新版本 `Origin.exe` 是否存在、启动日志和缺失模块错误。
...
```text
%LOCALAPPDATA%\Origin\installer\<version-pid-time>\install.log
```
