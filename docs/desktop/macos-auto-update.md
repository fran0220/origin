...
本文是 Origin Desktop macOS 更新链路的维护手册，与 [`windows-auto-update.md`](./windows-auto-update.md) 平行。两端共用同一套更新源配置、发布脚本、状态机和 CI 编排；差异集中在安装机制与产物形态，本文只展开 macOS 侧。
...
desktop/
  stable/
    latest-mac.yml
    Origin-<version>-mac.zip              # x64 更新包
    Origin-<version>-mac.zip.blockmap
    Origin-<version>.dmg                  # x64 首装包
    Origin-<version>-arm64-mac.zip        # arm64 更新包
    Origin-<version>-arm64-mac.zip.blockmap
    Origin-<version>-arm64.dmg            # arm64 首装包
```
...
2. 从终端启动以便看日志：`/Applications/Origin.app/Contents/MacOS/Origin`
...
| `state = running` 但版本没变 | ShipIt 在等目标进程退出——检查是不是还有 Origin 进程活着。 |
...
Moved bundle contents from ... to file:///Applications/Origin.app/
Installation completed successfully
Successfully launched application at file:///Applications/Origin.app/
```
...
**别用应用窗口判断应用是否还在跑。** 单实例锁会让新启动的实例退出并把老进程的窗口调出来，看着像「重启了但版本没变」，实际是同一个老进程。用 `pgrep -f "Origin.app/Contents/MacOS/Origin"` 看 pid，并和 `launchctl print` 里的 `submitted by Origin[pid]` 对照。
...
```
Origin.app/Contents/Resources/vendor/python/cpython-...tar.gz/cpython-...tar/python/bin/python3.13
...
打包应用的 stdout 只有从终端启动才看得到：`/Applications/Origin.app/Contents/MacOS/Origin`。
