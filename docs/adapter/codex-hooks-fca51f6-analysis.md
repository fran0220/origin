# Codex Hook 最新基线分析与 Origin 实施方案

## 1. 分析范围

本文只分析 Hook 兼容，不把 Origin 实现成 Codex，也不改变既定文件发现边界。

- 最新 Codex 源码：`C:\github\codex`
- 最新基线：`fca51f6dafb106177f23084d16f076ff2f66dd91`，2026-07-15
- Origin 兼容包：`packages/ecosystem-adapter`
- Origin 宿主接入：`packages/coding-agent/src/core/hooks`

本文以 `fca51f6` 为唯一兼容基线，不保留旧版 Codex Hook profile。

文件发现边界保持不变：兼容层只解析 Origin 应用显式提供的配置文件或插件目录，不搜索 `CODEX_HOME`、用户 `.codex`、项目 `.codex`，更不能运行时读取 `C:\github\codex`。
...
当前 Origin 对所有非空 matcher 都使用 `new RegExp()`，并不等价。例如 `mcp__memory` 在 Codex 中是精确匹配，在 Origin 中可能匹配更长名称。
...
这意味着 Origin 不能继续使用“只有 `toolName === "bash"` 才支持 Codex Tool Hook”的判断。也不能把所有 Origin 工具伪装成 Bash。正确方式是为工具调用提供可扩展的 Hook contract：工具实际名称、matcher aliases、稳定输入、稳定输出以及输入改写函数分别建模。
...
Origin 应支持“由 Origin 插件加载器显式传入的插件 Hook 来源”，但不能因此扫描外部 Codex 安装目录。插件根目录必须位于 Origin 已安装或已启用的应用插件范围，Hook 文件也必须经过根目录边界校验。
...
未启用或不受信任的普通 Hook 不进入执行集合。当前 Origin 缮信任状态和内容变更检测，属于安全和兼容双重缺口。
...
当前 Origin 把 additional context 和 feedback 拼进工具结果文本，并忽略 `post.shouldBlock`，与最新版语义不一致。
...
| MCP 名称 | Origin 使用 `mcp_<server>_<tool>` | 与 Codex `mcp__<server>__<tool>` 不兼容，且仅靠字符串无法可靠拆分含下划线名称 |
...
这些名称表达 Origin 的真实边界，不承诺任何外部 wire 字段。Codex adapter、未来 Claude adapter 或其他生态 adapter 分别把 Host event 映射为自己的事件。
...
- 文件发现由 Origin 应用 source provider 负责，adapter 只接收显式 source。
...
- Origin `bash`、`shell` -> Codex `Bash`。
- MCP adapter 在创建工具时保留 server/tool 原始元数据，再编码为 `mcp__server__tool`；不要从 `mcp_server_tool` 猜边界。
- 普通 function tool 默认使用真实名称和原 JSON 输入。
- Origin `edit`、`write` 先提供 matcher aliases `Edit`、`Write`。只有能生成等价 `apply_patch` 输入时，才能宣称 canonical `apply_patch` 精确兼容。
...
验证：Hook 不会绕过 Origin 未授权的宿主能力；PreCompact 停止时压缩没有开始；PostCompact 发生在新摘要和 transcript 已稳定之后。

### 阶段 5：Subagent 生命周期

1. 先建立 Origin 可见子代理的真实生命周期和父子 transcript 关系。
...
1. 解析 Origin 已安装插件 manifest 中显式声明的 Codex Hook 路径。
2. 提供四个插件环境变量。
...
10. 所有配置只从 Origin 应用显式提供的目录或已安装插件根目录生效。
...
| 普通 function tool | 真实扁平工具名 | 默认使用 Origin 工具真实名称和原始 JSON 参数 | 兼容 |
...
| `apply_patch` | canonical `apply_patch`，aliases `Write/Edit` | 真正名为 `apply_patch` 的工具可按真实名匹配；Origin `edit/write` 保留自身 canonical 并提供对应 alias | 部分兼容，不伪造不等价输入 |
...
3. Origin 插件 manifest 到 `HookConfigLayer.sources` 的自动桥接、插件路径越界检查。
...
7. Origin `edit/write` 与 Codex `apply_patch` 的输入格式转换；在存在可证明的等价转换前不宣称 canonical 完全兼容。
...
| `PreToolUse` 的 `Bash\|apply_patch\|Edit\|Write` | Bash canonical 匹配；Origin edit/write 通过 aliases 匹配 |
...
需要由集成方处理文件部署：Origin 不会扫描样本生成的 `.codex/hooks.json`。必须把 Hook 配置作为 Origin 应用显式 source 提供；配置中引用的 Python 脚本也必须位于该应用/插件已安装范围内，并让命令路径与实际部署位置一致。兼容层不会自动改写脚本路径或去外部 Codex 目录寻找文件。
