# oh-story-claudecode → Origin 无缝兼容方案
...
> 让 Origin 原生兼容 oh-story 已提供的 Codex/Claude Code 能力面，使上游 Skill、Agent、Hooks、脚本和命令尽量原样运行。

最终交付必须满足：

1. 上游 13 个 `SKILL.md` 不因 Origin 修改业务流程。
...
因此最佳路线不是再维护一套 `oh-story-vetta` 业务 fork，而是让 Origin 实现一个可复用的 **Codex Project Compatibility Profile**。oh-story 在 Origin 中选择 Codex 部署目标，上游文件直接成为 Origin 的项目资源。

## 推荐架构

```mermaid
flowchart TB
    B["oh-story 原始仓库快照"] --> P["Origin 资源插件"]
    P --> S["13 个原始 Skills"]
...
Origin 核心中不得出现 `story-*`、`正文/`、`细纲`、`伏笔` 等业务判断。所有小说规则继续由上游文件拥有。
...
- 不维护 Origin 专用的 13 份 `SKILL.md`。
- 不把 Hooks 重写成 `story_write_file` 等 Origin 专用工具。
- 不把 `browser-cdp` 重写成另一套 MCP 调用协议。
- 不把 `story-cover` 改成 Origin 专用提示词流程。
...
## 2. story-setup 在 Origin 中直接走 Codex 部署

Origin 会话需要公开结构化宿主信息：
...
上游仍可提示“新开会话”，但 Origin 应做到部署完成后立即显示 7 个 Agent 和 Hook 状态。
...
每个 Skill 自动注册 `/name` 与 `$name`，不要求逐个在 Origin 写别名。中文别名可由插件额外声明，但不从 description 自由文本中猜测。

### 模型工具兼容

保留 Origin 原生工具，同时增加兼容别名：
...
Origin 的 Read/Write/Edit/Glob/Grep/Bash 工具向兼容上下文提供 Claude/Codex 名称别名。Hook 匹配也使用这些规范名称，避免上游 matcher 因 Origin 内部小写工具名失效。
...
用户在 Origin 设置中为 low/medium/high 选择实际 provider/model。未配置时继承父模型并在 Agent 运行详情中提示，不因此降级 solo。
...
工具名在发送前做兼容映射：Origin `bash/shell` → `Bash`，`write` → `Write`，`edit` → `Edit`，`apply_patch` 保持原名。
...
`permissionDecision = "deny"` 映射到 Origin `tool_call` block/reason；`additionalContext` 进入当前上下文，不只显示为日志。

Hook 超时、退出码、stdout/stderr、匹配结果和阻断原因在活动面板可见。Hook 失败不得被当作“检查通过”。是否 fail-open 由事件和项目授权策略决定，并明确显示。

## 6. Windows POSIX 兼容运行时

仅靠 PowerShell 无法原样执行上游大量 Bash 示例。Origin 应提供可选的托管 POSIX runtime，并在 Codex compatibility profile 中暴露 `Bash` 工具。
...
普通 Origin PowerShell 工具保持不变；上游 Skill 调用 `Bash` 时进入托管 POSIX runtime。路径桥负责 Windows native path 与 `/c/...` 形式转换，并给子进程设置 UTF-8。

复杂命令不做 PowerShell 自动翻译。翻译 Shell 文本无法保证引号、管道、heredoc、`set -euo pipefail` 和退出码语义，是伪兼容。

## 7. browser-cdp 原协议支持

不再用 MCP 替换 `browser-cdp`。Origin 托管运行时直接提供上游要求的 `agent-browser` 可执行命令，并保证其在 `Bash` 与 Windows 命令环境 PATH 中可见。
...
上游脚本已经覆盖 Windows Chrome 路径和 `commandWindows`，因此优先保持它，而不是另写 Origin 浏览器协议。未来可以提供不关闭常规 Chrome 的增强模式，但不能让增强模式破坏原始 CLI 行为。

## 8. story-cover 原协议支持

目标是保留上游 `curl + OpenAI Images API + b64_json + 文件落盘 + ImageMagick` 流程，同时复用 Origin 图像配置。
...
1. Origin 为当前会话生成短期 bearer token。
2. 向兼容 Shell 环境注入：
...
平台上传尺寸继续执行上游 Step 3.5。托管 runtime 提供 ImageMagick，保证 `600×800` 居中裁剪、缩放和 `_上传.png` 落盘，无需新增 Origin 专用 `images.export` 工具。
...
- 超时后杀掉完整进程树。
- Origin 本地 image token 只允许当前会话和图像路由。
...
| `apps/desktop/src/main/` | 项目信任、进程树、托管 POSIX runtime、image bridge |
| `apps/desktop/src/renderer/` | Agent/Hook 状态、授权和诊断 UI，所有用户文案接 i18n |
...
| `story-setup` | 原样完整 | Origin 作为 Codex-compatible target |
...
- 构建与校验脚本。
- 上游尚未合并的极小 Origin 宿主识别补丁。

13 个业务 Skill、7 个 Agent 指令、Hook 脚本和 references 不进入 Origin 私有 patch 队列。若兼容测试失败，优先修复 Origin compatibility profile，而不是改写上游 Skill。
...
正确边界是：Origin 补齐 Codex-compatible 宿主能力，上游业务包保持原样。只有在上游协议本身无法表达 Origin 宿主身份时，才维护极小、可回馈上游的识别补丁。
